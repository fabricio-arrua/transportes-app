...
export default function HomeTecnico() {
  ...
  const handleOpenPopup = async (matricula) => {
    ...
    try {
      const response = await axios.get(`http://localhost:4000/api/camiones/listarHistorialMantenimientoCamion`, {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:4000/api/camiones/listarCamionesEnReparacionAgrupado`, {
      headers: {
        Authorization: cookies.get('token'), 
      }})
      .then((response) => {
